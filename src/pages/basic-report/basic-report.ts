...
export class BasicReportPage extends BasePage {
	...
	showActions(person: IPersonReadonly, personMath: IMathBasicFinancer) {
		...
		this.presentActions({
			title   : this.translate('BASIC_REPORT_PAGE.ACTION_TITLE', { name: person.name }),
			buttons : buttons,
		})
	}
	...
	async returnChange(person: IPersonReadonly, personMath: IMathBasicFinancer) {
		...
		await this.presentAlert({
			...
			message : this.translate('BASIC_REPORT_PAGE.RETURN_CHANGE_MESSAGE', data),
			inputs  : [{ type: 'number', name: 'amount', placeholder: this.translate('BASIC_REPORT_PAGE.RETURN_CHANGE_PLACEHOLDER', data), value: data.maxChange }],
			buttons : [
				...
				this.ALERT_BUTTONS.RETURN_CHANGE.onBeforeDismiss(async(data) => {
					...
					if (!ValidationHelpers.isZeroOrPositive(amount, false)) {
						this.presentToast({ message: this.translate('BASIC_REPORT_PAGE.ERR_INVALID_AMOUNT'), duration: 3000 });
						return false;
					}
					if (amount > personMath.change) {
						this.presentToast({ message: await this.loadTranslations('BASIC_REPORT_PAGE.ERR_UNFAIR_CHANGE', { name: person.name, amount: personMath.change.toFixed(2).replace('.00','') }), duration: 3000 });
						return false;
					}
					if (amount > maxValue) {
						this.presentToast({ message: this.translate('BASIC_REPORT_PAGE.ERR_INSUFFICIENT_CHANGE'), duration: 3000 });
						return false;
					}
	...
	async poolAmount(person: IPersonReadonly, personMath: IMathBasicFinancer) {
		...
		await this.presentAlert({
			title   : this.translate('BASIC_REPORT_PAGE.POOL_AMOUNT_TITLE', data),
			message : this.translate('BASIC_REPORT_PAGE.POOL_AMOUNT_MESSAGE', data),
			inputs  : [{ type: 'number', name: 'amount', placeholder: this.translate('BASIC_REPORT_PAGE.POOL_AMOUNT_PLACEHOLDER', data), value: person.amountPooled ? data.amountPooled : '' }],
			buttons : [
				this.ALERT_BUTTONS.CANCEL,
	...
	async rename(person: IPersonReadonly) {
		...
		await this.presentAlert({
			title   : this.translate('BASIC_REPORT_PAGE.RENAME_TITLE', data),
			message : this.translate('BASIC_REPORT_PAGE.RENAME_MESSAGE', data),
			inputs  : [{ type: 'text', name: 'name', placeholder: this.translate('BASIC_REPORT_PAGE.RENAME_PLACEHOLDER', data) }],
			buttons : [
				this.ALERT_BUTTONS.CANCEL,
				this.ALERT_BUTTONS.RENAME.onBeforeDismiss(data => {
					...
					if (!newName) {
						this.presentToast({ message: this.translate('BASIC_REPORT_PAGE.ERR_BLANK_NAME'), duration: 3000 });
						return false;
					}
					if (newName !== oldName && this.split.personList.hasName(newName)) {
						this.presentToast({ message: this.translate('BASIC_REPORT_PAGE.ERR_DUPLICATE_NAME'), duration: 3000 });
						return false;
					}
